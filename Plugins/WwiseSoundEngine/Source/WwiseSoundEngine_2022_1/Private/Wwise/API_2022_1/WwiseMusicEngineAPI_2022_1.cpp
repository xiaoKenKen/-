...
AKRESULT FWwiseMusicEngineAPI_2022_1::Init(
	AkMusicSettings* in_pSettings
)
{
	SCOPED_WWISESOUNDENGINE_EVENT(TEXT("AK::MusicEngine::Init"));
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	return AK::MusicEngine::Init(in_pSettings);
}

void FWwiseMusicEngineAPI_2022_1::GetDefaultInitSettings(
...
void FWwiseMusicEngineAPI_2022_1::Term(
)
{
	SCOPED_WWISESOUNDENGINE_EVENT(TEXT("AK::MusicEngine::Term"));
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	AK::MusicEngine::Term();
}

AKRESULT FWwiseMusicEngineAPI_2022_1::GetPlayingSegmentInfo(
