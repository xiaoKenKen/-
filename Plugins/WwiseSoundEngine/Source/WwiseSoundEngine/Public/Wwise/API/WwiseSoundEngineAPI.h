...
{
public:
	static IWwiseSoundEngineAPI* Get()
	{
		IWwiseSoundEngineModule::ForceLoadModule();
		return IWwiseSoundEngineModule::SoundEngine;
	}

	class IQuery;
	class IAudioInputPlugin;
	class IDynamicDialogue;
	class IDynamicSequence;
	
	UE_NONCOPYABLE(IWwiseSoundEngineAPI);
protected:
	IWwiseSoundEngineAPI(
		IQuery* InQuery,
		IAudioInputPlugin* InAudioInputPlugin,
		IDynamicDialogue* InDynamicDialogue,
		IDynamicSequence* InDynamicSequence) :
		Query(InQuery),
		AudioInputPlugin(InAudioInputPlugin),
		DynamicDialogue(InDynamicDialogue),
		DynamicSequence(InDynamicSequence)
	{}
public:
	virtual ~IWwiseSoundEngineAPI()
	{
		delete Query;
		delete AudioInputPlugin;
		delete DynamicDialogue;
		delete DynamicSequence;
	}
	...
	virtual AK::IAkGlobalPluginContext * GetGlobalPluginContext() = 0;
	...
	virtual AKRESULT UnregisterAudioDeviceStatusCallback() = 0;
	//@}
	...
	virtual AkUInt32 GetIDFromString( const wchar_t* in_pszString ) = 0;
	...
	/// - <tt>AK::SoundEngine::PrepareEvent</tt>
	/// - <tt>AK::SoundEngine::PrepareGameSyncs</tt>
	virtual AkUInt32 GetIDFromString( const char* in_pszString ) = 0;
	...
	virtual bool GetBackgroundMusicMute () = 0;
	//@}
	...
	virtual AKRESULT ClearPreparedEvents() = 0;
	...
	virtual AKRESULT StopOutputCapture() = 0;
	...
	virtual AkUInt32 GetSampleRate() = 0;
	...
	/// \remark This function is provided as a utility tool only. It does nothing if it is 
	///			called in the release configuration and returns AK_NotCompatible.
	virtual AKRESULT StopProfilerCapture() = 0;
	...
	public:
		...
		using FAkGameObjectsList = TArray<AkGameObjectID>;
		...
		using FAkRadiusList = TArray<AK::SoundEngine::Query::GameObjDst>;

		/// Returns the maximum distance used in attenuation associated to all sounds currently playing.
	...
	{
	public:
		UE_NONCOPYABLE(IAudioInputPlugin);

	protected:
		IAudioInputPlugin() = default;

	public:
		virtual ~IAudioInputPlugin() {}

		virtual void SetAudioInputCallbacks(
	...
	{
	public:
		UE_NONCOPYABLE(IDynamicDialogue);

	protected:
		IDynamicDialogue() = default;

	public:
		virtual ~IDynamicDialogue() {}
	...
	{
	public:
		UE_NONCOPYABLE(IDynamicSequence);

	protected:
		IDynamicSequence() = default;

	public:
		virtual ~IDynamicSequence() {}

		class IPlaylist
		{
			UE_NONCOPYABLE(IPlaylist);
		...
		public:
			...
			/// Removes all PlaylistItem in the array. Optionally reserves space for multiple items.
			virtual void RemoveAll(int Reserve = 0) = 0;

			/// Insert a PlaylistItem at the specified position without filling it.
			/// Returns the pointer to the PlaylistItem to be filled.
			virtual AK::SoundEngine::DynamicSequence::PlaylistItem* Insert(unsigned int in_uIndex) = 0;

			/// Erase the PlaylistItem at the specified index
			virtual void Erase(unsigned int in_uIndex) = 0;

			/// Operator [], return a reference to the specified index.
			virtual AK::SoundEngine::DynamicSequence::PlaylistItem& operator[](unsigned int uiIndex) const = 0;
			...
			virtual AK::SoundEngine::DynamicSequence::Playlist& GetPlaylist() = 0;

			/// Returns the playlist.
			virtual const AK::SoundEngine::DynamicSequence::Playlist& GetPlaylist() const = 0;
		};
	...
	class TagInformationBridge : private AkErrorMessageTranslator
	{
	public:
		using Type = TagInformation;
	};
#else
	class TagInformationBridge
	{
	public:
	...
	using FGetInfoErrorMessageTranslatorFunction = TFunction<bool(TagInformationBridge::Type* in_pTagList, AkUInt32 in_uCount, AkUInt32& out_uTranslated)>;
	virtual AkErrorMessageTranslator* NewErrorMessageTranslator(FGetInfoErrorMessageTranslatorFunction InMessageTranslatorFunction) = 0;
};
