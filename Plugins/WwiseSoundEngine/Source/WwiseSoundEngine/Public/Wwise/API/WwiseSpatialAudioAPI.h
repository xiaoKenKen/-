...
public:
	...
	virtual AKRESULT SetTransmissionOperation(AkTransmissionOperation in_operation) = 0;

	/// Enable parameter smoothing on the diffraction paths output from the Acoustics Engine, either globally or for a specific game object. 
	///
	virtual AKRESULT SetSmoothingConstant(AkReal32 in_smoothingConstant, AkGameObjectID in_gameObject) = 0;
	...
	///
	virtual AKRESULT SetMaxGlobalReflectionPaths(AkUInt32 in_maxNumberOfReflectionPaths) = 0;
	
	/// Set the maximum number of computed diffraction paths, either for a particular Game Object or for all Game Objects.
	///
	virtual AKRESULT SetMaxDiffractionPaths(AkUInt32 in_maxNumberOfDiffractionPaths, AkGameObjectID in_gameObject) = 0;
