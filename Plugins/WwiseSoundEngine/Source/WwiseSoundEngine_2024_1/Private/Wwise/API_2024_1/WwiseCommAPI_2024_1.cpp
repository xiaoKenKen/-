...
AKRESULT FWwiseCommAPI_2024_1::Init(
	const AkCommSettings& in_settings
)
{
	SCOPED_WWISESOUNDENGINE_EVENT(TEXT("AK::Comm::Init"));
...
AkInt32 FWwiseCommAPI_2024_1::GetLastError()
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
	return 0;
}

void FWwiseCommAPI_2024_1::GetDefaultInitSettings(
...
void FWwiseCommAPI_2024_1::Term()
{
	SCOPED_WWISESOUNDENGINE_EVENT(TEXT("AK::Comm::Term"));
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
#if AK_ENABLE_COMMUNICATION
	AK::Comm::Term();
#else
...
AKRESULT FWwiseCommAPI_2024_1::Reset()
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
#if AK_ENABLE_COMMUNICATION
...
const AkCommSettings& FWwiseCommAPI_2024_1::GetCurrentSettings()
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
#if AK_ENABLE_COMMUNICATION
...
AkUInt16 FWwiseCommAPI_2024_1::GetCommandPort()
{
	SCOPE_CYCLE_COUNTER(STAT_WwiseSoundEngineAPI);
#if AK_ENABLE_COMMUNICATION
