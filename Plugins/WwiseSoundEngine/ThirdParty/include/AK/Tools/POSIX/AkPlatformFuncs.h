...
#if !defined(AK_EMSCRIPTEN) && !defined(AK_APPLE)
		// Set thread name for platforms supporting standard pthread_setname_np
		// failure here is not critical
		if (in_szThreadName)
		{
			pthread_setname_np(*out_pThread, in_szThreadName);
		}
#endif
    }
#endif
