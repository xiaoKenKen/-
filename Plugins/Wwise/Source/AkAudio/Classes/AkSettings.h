...
public:
	...
	*/
	...
	TWeakObjectPtr<UAkAcousticTexture> GetAssociatedAcousticTexture(const TWeakObjectPtr<UPhysicalMaterial> InPhysMaterial) const;
	uint32 GetAssociatedAcousticTextureID(const TWeakObjectPtr<UPhysicalMaterial> InPhysMaterial) const;
	float GetAssociatedOcclusionValue(const TWeakObjectPtr<UPhysicalMaterial> InPhysMaterial) const;
	...
	void CreateGeometrySurfacePropertiesTable();
	void VerifyAndUpdateGeometrySurfacePropertiesTable();
	void CreateReverbAssignmentTable();
...
private:
	...
	TWeakObjectPtr<UDataTable> CreateAndAssignGeometrySurfacePropertiesTable();
	void MigrateAkGeometryMap(TWeakObjectPtr<UDataTable> inout_GeometryMap);
	...
	TWeakObjectPtr<UDataTable> CreateAndAssignReverbAssignmentTable();
	void MigrateEnvironmentDecayAuxBusMap(TWeakObjectPtr<UDataTable> inout_DecayTable);
