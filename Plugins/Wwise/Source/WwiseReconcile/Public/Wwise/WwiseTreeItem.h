...
struct WWISERECONCILE_API FWwiseTreeItem : public TSharedFromThis<FWwiseTreeItem>
{
private:
...
public:
	...
	FString FolderPath;
	/** Type of the item */
	EWwiseItemType ItemType = EWwiseItemType::None;
	/** Id of the item */
	FGuid ItemId;
	/** ShortId of the item*/
	uint32 ShortId = 0;
	...
	TSharedPtr<WwiseMetadataBasicReference> WwiseItemRef;
#endif
	...
	/** Constructor */
	FWwiseTreeItem(FString InDisplayName, FString InFolderPath, TSharedPtr<FWwiseTreeItem> InParent, EWwiseItemType InItemType, const FGuid& InItemId, uint32 InShortId = AK_INVALID_UNIQUE_ID)
...
#if WITH_EDITORONLY_DATA
	FWwiseTreeItem(const WwiseMetadataBasicReference& ItemRef, TSharedPtr<FWwiseTreeItem> InParent, EWwiseItemType InItemType)
		: ItemType(InItemType)
		, ChildCountInWwise(m_Children.Num())
		, Parent(MoveTemp(InParent))
	{
		WwiseItemRef = MakeShared<WwiseMetadataBasicReference>(ItemRef.Id, ItemRef.Name, ItemRef.ObjectPath, ItemRef.GUID);
		int A, B, C, D;
		WwiseItemRef->GUID.GetGuidValues(A, B, C, D);
		ItemId = FGuid(A, B, C, D);
		DisplayName = FWwiseStringConverter::ToFString(WwiseItemRef->Name);
		ShortId = ItemRef.Id;
		FolderPath = FWwiseStringConverter::ToFString(WwiseItemRef->ObjectPath);
	}
#endif
	...
	bool IsOfType(const TArray<EWwiseItemType>& Types) const;

	bool IsNotOfType(const TArray<EWwiseItemType>& Types) const;
