...
FWwiseAssetLibraryPreCooker::FAssetLibraryInfoMap FWwiseAssetLibraryPreCooker::Process(const FAssetLibraryArray& AssetLibraryArray)
{
	...
	for (int Iter = 0; Iter < AssetLibraryArray.Num(); ++Iter)
	{
		...
		{
			UE_LOG(LogWwisePackagingEditor, Log, TEXT("FWwiseAssetLibraryPreCooker::Process: Could not retrieve Asset Library at location %d (%s). Skipping."), Iter, *AssetLibrarySoftPtr.Get()->GetName());
			continue;
		}
		...
		FWwiseAssetLibraryProcessor::GetRelevantAssets(AssetLibrary->RelevantGameplayModulePaths, FilteringSharedData->AssetsData, AssetLibrary->ExcludedGameplayModulePaths);
