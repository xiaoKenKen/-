...
void FWwiseMediaManagerImpl::DoTerm()
{
	...
	for (auto FileStateTuple : FileStatesById)
	{
		...
		bool bIsStreamedWithPrefetch = false;
		
		if (FileState.State == FWwiseFileState::EState::Loaded)
		{
			bShouldUnload = true;
		}
		else if (FileState.State == FWwiseFileState::EState::Opened && FileState.IsStreamedState())
		{
			auto* StreamedMediaFileState = FileState.GetStateAs<FWwiseStreamedMediaFileState>();
			if (StreamedMediaFileState && StreamedMediaFileState->HasPrefetchDataInMemory())
			{
				bShouldUnload = true;
				bIsStreamedWithPrefetch = true;
			}
		}
