	...
	virtual void BuildSectionContextMenu(FMenuBuilder& MenuBuilder, const FGuid& ObjectBinding) override
	{
		MenuBuilder.BeginSection("Audiokinetic", LOCTEXT("AKMenu", "Audiokinetic"));
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateLambda([this] { return SnapSectionLengthToEventLength(); }),
					FCanExecuteAction::CreateLambda([this] { return Section != nullptr; })
				),
				NAME_None,
...
void FMovieSceneAkAudioEventTrackEditor::BuildTrackContextMenu(FMenuBuilder& MenuBuilder, UMovieSceneTrack* Track)
{
	...
	{
		MenuBuilder.AddMenuEntry(
		   LOCTEXT("RefreshAllWaveforms", "Saves the Wwise project and refreshes all sections."),
...
bool FMovieSceneAkAudioEventTrackEditor::HandleAssetAdded(UObject* Asset, const FGuid& TargetObjectGuid)
{
	if (Asset->IsA<UAkAudioEvent>())
	{
		...
		else
		{
			...
			{
#if UE_5_2_OR_LATER
				auto TrackResult = FindOrCreateRootTrack<UMovieSceneAkAudioEventTrack>();
#else
...
#if UE_5_2_OR_LATER
			auto NewTrack = FocusedMovieScene->AddTrack<UMovieSceneAkAudioEventTrack>();
#else
